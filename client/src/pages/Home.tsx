...
type DailyRow = { f?: number; fab?: number; fcc?: number; fcn?: number; fcp?: number; cat?: number[] };
...
function metricsFromSheet(rows: SheetRow[]): Metrics {
  ...
  for (const [day, row] of Object.entries(daily)) serializedDaily[day] = { f: row.f, fab: row.fab, fcc: row.fcc, fcn: row.fcn, fcp: row.fcp, cat: categories.map((name) => Number(row._categories?.[name] || 0)) };
