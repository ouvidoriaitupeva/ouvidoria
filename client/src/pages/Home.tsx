...
import { BarChart3, Check, Clock3, ShieldCheck, Target, Upload } from "lucide-react";

type DailyRow = { f?: number; fab?: number; fcc?: number; fcn?: number; fcp?: number; cat?: number[]; ass?: number[]; assC?: number[]; assA?: number[] };
type Metrics = { meta?: { gerado_em?: string; janela_inicio?: string; janela_fim?: string; categorias?: string[]; secretarias?: string[]; assuntos?: string[]; fonte?: string }; diario?: Record<string, DailyRow>; mensal_dim?: Record<string, { sec?: { reg?: number[]; cc?: number[] }; ass?: { reg?: number[] } }>; abertos_mais_30d?: { total?: number } };
...
export default function Home() {
  ...
  const [sourceName, setSourceName] = useState("metricas.json");
  ...
  const handleUpload = (event: React.ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; file.arrayBuffer().then((buffer) => { const workbook = XLSX.read(buffer, { type: "array", cellDates: true }); const sheet = workbook.Sheets[workbook.SheetNames[0]]; const rows = XLSX.utils.sheet_to_json<SheetRow>(sheet, { defval: "" }); setData(metricsFromSheet(rows)); setSourceName(file.name); setLoadError(false); }).catch(() => setLoadError(true)); };
  ...
  const secretariatRows = secretariats.map((name, index) => { let reg = 0; let cc = 0; periodMonths.forEach((monthKey) => { const month = data?.mensal_dim?.[monthKey]; reg += Number(month?.sec?.reg?.[index] || 0); cc += Number(month?.sec?.cc?.[index] || 0); }); return { name, reg, cc, open: Math.max(0, reg - cc) }; }).filter((row) => row.reg > 0).sort((a, b) => b.reg - a.reg);
