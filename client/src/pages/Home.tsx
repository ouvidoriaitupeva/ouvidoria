...
type DailyRow = { f?: number; fab?: number; fcc?: number; fcn?: number; fca?: number; fcp?: number; fco?: number; cat?: number[]; ass?: number[]; assC?: number[]; assA?: number[] };
type CanonicalStatus = "Em tratamento" | "Respondidas" | "Arquivadas" | "Canceladas" | "Reabertas" | "Não mapeado";
type Metrics = { meta?: { gerado_em?: string; janela_inicio?: string; janela_fim?: string; categorias?: string[]; secretarias?: string[]; assuntos?: string[]; fonte?: string; status_map?: Record<string, CanonicalStatus>; status_nao_mapeados?: string[]; registros_considerados?: number; registros_excluidos?: number; indicadores_indisponiveis?: string[] }; diario?: Record<string, DailyRow>; mensal_dim?: Record<string, { sec?: { reg?: number[]; cc?: number[]; open?: number[]; within?: number[]; outside?: number[]; cat?: number[][] }; ass?: { reg?: number[] } }>; abertos_mais_30d?: { total?: number }; tempo_medio?: { geral?: number | null; categorias?: (number | null)[]; secretarias?: (number | null)[]; assuntos?: (number | null)[]; base?: string; registros_no_periodo?: number; registros_finalizados?: number; registros_finalizados_com_tempo?: number; registros_excluidos_por_data?: number; diario?: Record<string, { geral?: number | null; geral_count?: number; categorias?: (number | null)[]; categorias_count?: number[]; secretarias?: (number | null)[]; secretarias_count?: number[]; assuntos?: (number | null)[]; assuntos_count?: number[] }>; mensal?: Record<string, { geral?: number | null; geral_count?: number; categorias?: (number | null)[]; categorias_count?: number[]; secretarias?: (number | null)[]; secretarias_count?: number[]; assuntos?: (number | null)[]; assuntos_count?: number[] }> } };
...
function normalizeStatus(value: unknown): CanonicalStatus {
  const original = String(clean(value) || "").trim();
  const key = original.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
  if (["concluido", "concluida", "respondido", "respondida", "finalizado", "finalizada", "resolvido", "resolvida"].includes(key)) return "Respondidas";
  if (["arquivado", "arquivada"].includes(key)) return "Arquivadas";
  if (["cancelado", "cancelada"].includes(key)) return "Canceladas";
  if (["reaberto", "reaberta"].includes(key)) return "Reabertas";
  if (["em andamento", "aberto", "aberta", "pendente", "agendado", "agendada", "em analise", "em tratamento"].includes(key)) return "Em tratamento";
  return "Não mapeado";
}
...
export function metricsFromSheet(rows: SheetRow[]): Metrics {
  ...
  const monthlySecretariat: Record<string, Record<string, { reg: number; cc: number; open: number; within: number; outside: number; categories: Record<string, number> }>> = {};
  ...
  for (const row of rows) {
    ...
    const isOverdueOpen = Boolean(overdueCutoff && received < overdueCutoff && status !== "Respondidas" && status !== "Canceladas");
    ...
    if (status === "Respondidas") item._subjectC![subject] = Number(item._subjectC![subject] || 0) + 1;
    else if (status !== "Canceladas") item._subjectA![subject] = Number(item._subjectA![subject] || 0) + 1;
    if (status === "Respondidas") { item.fcc = Number(item.fcc || 0) + 1; if (responseDays != null) { if (responseDays > 30) item.fco = Number(item.fco || 0) + 1; else item.fcp = Number(item.fcp || 0) + 1; } }
    else if (status === "Canceladas") item.fcn = Number(item.fcn || 0) + 1;
    else item.fab = Number(item.fab || 0) + 1;
    if (!monthlySecretariat[month]) monthlySecretariat[month] = {};
    if (!monthlySecretariat[month][secretariat]) monthlySecretariat[month][secretariat] = { reg: 0, cc: 0, open: 0, within: 0, outside: 0, categories: {} };
    monthlySecretariat[month][secretariat].reg += 1;
    monthlySecretariat[month][secretariat].categories[category] = Number(monthlySecretariat[month][secretariat].categories[category] || 0) + 1;
    if (status === "Respondidas") monthlySecretariat[month][secretariat].cc += 1;
    else if (status !== "Canceladas") monthlySecretariat[month][secretariat].open += 1;
...
export default function Home() {
  ...
  const [period, setPeriod] = useState(() => localStorage.getItem("falabr-period") || "Últimos 12 meses");
  ...
  useEffect(() => {
    ...
    const load = () => {
      ...
      if (active) { setData(null); setLoadError(false); }
    };
    load();
  ...
  useEffect(() => {
    const syncPeriod = () => setPeriod(localStorage.getItem("falabr-period") || "Últimos 12 meses");
    window.addEventListener("falabr-period-updated", syncPeriod);
    return () => window.removeEventListener("falabr-period-updated", syncPeriod);
  }, []);
  const daily = data?.diario ?? {};
  const allDates = Object.keys(daily).sort();
  const latest = allDates[allDates.length - 1] ?? "";
  const range = useMemo(() => { if (!latest) return { start: "", end: "" }; if (period === "Período personalizado") return { start: start || latest, end: end || latest }; const latestDate = new Date(`${latest}T00:00:00`); const months = period === "Últimos 6 meses" ? 5 : period === "Últimos 3 meses" ? 2 : period === "Este mês" ? 0 : 11; return period === "Todo o período" ? { start: allDates[0], end: latest } : { start: `${isoDate(addMonths(latestDate, -months)).slice(0, 7)}-01`, end: latest }; }, [latest, period, start, end]);
