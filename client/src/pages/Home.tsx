...
type DailyRow = { f?: number; fab?: number; fcc?: number; fcn?: number; fcp?: number; cat?: number[]; ass?: number[] };
...
function metricsFromSheet(rows: SheetRow[]): Metrics {
  const daily: Record<string, DailyRow & { _categories?: Record<string, number>; _subjects?: Record<string, number> }> = {};
  ...
  for (const row of rows) {
    ...
    if (!daily[day]) daily[day] = { f: 0, fab: 0, fcc: 0, fcn: 0, cat: [], ass: [], _categories: {}, _subjects: {} };
  ...
  for (const [day, row] of Object.entries(daily)) serializedDaily[day] = { f: row.f, fab: row.fab, fcc: row.fcc, fcn: row.fcn, fcp: row.fcp, cat: categories.map((name) => Number(row._categories?.[name] || 0)), ass: subjects.map((name) => Number(row._subjects?.[name] || 0)) };
