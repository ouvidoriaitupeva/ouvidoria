...
type DailyRow = { f?: number; fab?: number; fcc?: number; fcn?: number; cat?: number[] };
...
function metricsFromSheet(rows: SheetRow[]): Metrics {
  const daily: Record<string, DailyRow & { _categories?: Record<string, number> }> = {};
  ...
  for (const row of rows) {
    ...
    if (!daily[day]) daily[day] = { f: 0, fab: 0, fcc: 0, fcn: 0, cat: [], _categories: {} };
    const item = daily[day];
    item.f = Number(item.f || 0) + 1;
    item._categories![category] = Number(item._categories![category] || 0) + 1;
    if (status === "Concluído") item.fcc = Number(item.fcc || 0) + 1;
  ...
  for (const [day, row] of Object.entries(daily)) serializedDaily[day] = { f: row.f, fab: row.fab, fcc: row.fcc, fcn: row.fcn, cat: categories.map((name) => Number(row._categories?.[name] || 0)) };
...
export default function Home() {
  ...
  const secretariatRows = secretariats.map((name, index) => { let reg = 0; let cc = 0; selectedDates.forEach((date) => { const month = data?.mensal_dim?.[date.slice(0, 7)]; reg += Number(month?.sec?.reg?.[index] || 0); cc += Number(month?.sec?.cc?.[index] || 0); }); return { name, reg, cc }; }).filter((row) => row.reg > 0).sort((a, b) => b.reg - a.reg).slice(0, 6);
