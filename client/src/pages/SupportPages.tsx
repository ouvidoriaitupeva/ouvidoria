...
type ReportMetric = { name: string; registered: number; concluded: number; open: number; outside: number | null; resolution: number | null };
function exportReport(metrics: ReportMetric[], period: string) {
  const rows = [["Secretaria", "Registrados", "Concluídos", "Abertos", "Fora do prazo", "Resolutividade"], ...metrics.map((item) => [item.name, item.registered, item.concluded, item.open, item.outside ?? "", item.resolution == null ? "" : `${item.resolution.toFixed(1).replace(".", ",")}%`])];
  const csv = rows.map((row) => row.map((value) => `"${String(value).replace(/"/g, '""')}"`).join(";")).join("\r\n");
  const blob = new Blob(["\\ufeff", csv], { type: "text/csv;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = `relatorio-secretarias-${period.toLowerCase().replace(/[^a-z0-9]+/gi, "-") || "periodo"}.csv`;
...
export function ReportsPage() {
  ...
  return <OuvidoriaLayout><div className="mx-auto max-w-[1200px]"><PageTitle eyebrow="Relatórios" title="Relatórios por secretaria" description="Acompanhe o volume de manifestações e o cumprimento dos prazos por secretaria no período selecionado." /><div className="mb-6 flex flex-col items-stretch justify-end gap-2 sm:items-end"><select value={period} onChange={(e) => setPeriod(e.target.value)} className="h-11 rounded-lg border border-[#DCE3E4] bg-white px-3 text-xs font-bold"><option>Últimos 12 meses</option><option>Últimos 6 meses</option><option>Últimos 3 meses</option><option>Este mês</option><option>Todo o período</option><option>Período personalizado</option></select>{period === "Período personalizado" && <div className="flex gap-2"><input type="date" value={start} onChange={(e) => setStart(e.target.value)} aria-label="Data inicial" className="h-9 rounded-lg border border-[#DCE3E4] bg-white px-2 text-xs" /><input type="date" value={end} onChange={(e) => setEnd(e.target.value)} aria-label="Data final" className="h-9 rounded-lg border border-[#DCE3E4] bg-white px-2 text-xs" /></div>}</div><div className="mb-5 flex items-center justify-between gap-4 border-b border-[#DCE3E4] pb-4"><div><h2 className="text-sm font-extrabold uppercase tracking-[0.14em] text-[#173B5E]">Indicadores do período</h2><p className="mt-1 text-xs text-[#71818B]">Os valores são atualizados a partir da planilha carregada.</p></div><div className="hidden items-center gap-4 text-xs text-[#71818B] sm:flex"><span className="flex items-center gap-2"><i className="h-2.5 w-2.5 rounded-full bg-[#D94736]" />Fora do prazo</span></div></div><div className="overflow-hidden rounded-2xl border border-[#DCE3E4] bg-white shadow-[0_8px_24px_rgba(23,59,94,0.04)]"><div className="grid min-w-[900px] grid-cols-[minmax(280px,1.7fr)_repeat(5,minmax(105px,1fr))] gap-4 border-b border-[#DCE3E4] bg-[#F5F8F8] px-6 py-3 text-[10px] font-bold uppercase tracking-[0.12em] text-[#71818B] xl:grid"><div>Secretaria</div><div>Registrados</div><div>Concluídos</div><div>Abertos</div><div>Fora do prazo</div><div>Resolutividade</div></div><div className="divide-y divide-[#E8EEED]">
{metrics.map((item) => <div key={item.name} className="grid min-w-[900px] grid-cols-[minmax(280px,1.7fr)_repeat(4,minmax(105px,1fr))_minmax(110px,.8fr)] items-center gap-4 px-6 py-4 transition-colors hover:bg-[#F8FAFA]">
<div className="min-w-0"><h2 className="truncate text-sm font-semibold text-[#173B5E]">{item.name}</h2></div>
<div className="text-right tabular-nums text-sm font-semibold text-[#334E68]">{item.registered.toLocaleString("pt-BR")}</div>
<div className="text-right tabular-nums text-sm font-semibold text-[#334E68]">{item.concluded.toLocaleString("pt-BR")}</div>
<div className="text-right tabular-nums text-sm font-semibold text-[#334E68]">{item.open.toLocaleString("pt-BR")}</div>
<div className="text-right tabular-nums text-sm font-semibold text-[#B53B2F]">{item.outside === null ? "—" : item.outside.toLocaleString("pt-BR")}</div>
<div className="text-right tabular-nums text-sm font-semibold text-[#173B5E]">{item.resolution === null ? "—" : (item.resolution.toFixed(1).replace(".", ",") + "%")}</div>
</div>)}
</div></div>{metrics.length === 0 && <div className="rounded-2xl border border-dashed border-[#B8C8CE] bg-white/70 p-10 text-center text-sm text-[#71818B]">Carregando dados da fonte local…</div>}<div className="mt-7 flex justify-end"><ActionButton onClick={() => exportReport(metrics, period)}><Download size={16} /> Exportar relatório agregado</ActionButton></div></div></OuvidoriaLayout>; }
